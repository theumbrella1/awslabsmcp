...
class TestCache:
    ...
    @patch('awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils.doc_fetcher.parse_llms_txt')
    @patch('awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils.text_processor.normalize')
    @patch('awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils.text_processor.index_title_variants')
    def test_load_links_only(self, mock_index_variants, mock_normalize, mock_parse_llms):
        ...
        assert cache._URL_TITLES['https://example.com/overview'] == 'Overview'
    ...
    @patch('awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils.doc_fetcher.fetch_and_clean')
    @patch('awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils.text_processor.format_display_title')
    ...
    def test_ensure_page_returns_cached(self):
        ...
        with patch(
            'awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils.doc_fetcher.fetch_and_clean'
        ) as mock_fetch:
            # Act
            result = cache.ensure_page(test_url)
