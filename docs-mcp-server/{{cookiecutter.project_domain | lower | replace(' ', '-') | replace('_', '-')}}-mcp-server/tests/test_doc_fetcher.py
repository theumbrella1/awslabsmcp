...
from awslabs.{{cookiecutter.project_domain | lower | replace(' ', '_') | replace('-', '_')}}_mcp_server.utils import doc_fetcher
...
class TestDocFetcher:
    ...
    def test_parse_llms_txt_extracts_links(self):
        ...
        [Getting Started](https://example.com/doc1)
        [API Reference](https://example.com/doc2)
        ...
        with patch.object(doc_fetcher, '_get', return_value=llms_content):
            # Act
            result = doc_fetcher.parse_llms_txt('https://example.com/llm.txt')

            # Assert
            assert len(result) == 2
            assert ('Getting Started', 'https://example.com/doc1') in result
            assert ('API Reference', 'https://example.com/doc2') in result

    def test_parse_llms_txt_handles_empty_titles(self):
        """Test parse_llms_txt handles links with empty titles."""
        # Arrange
        llms_content = '[]({{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc1) [Title]({{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc2)'

        with patch.object(doc_fetcher, '_get', return_value=llms_content):
            # Act
            result = doc_fetcher.parse_llms_txt('{{cookiecutter.allowed_domains.split(",")[0].strip()}}/llms.txt')

            # Assert
            assert len(result) == 1  # Empty title links are filtered out
            assert ('Title', '{{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc2') in result
    ...
    def test_fetch_and_clean_html_content(self, mock_get):
        ...
        # Act
        result = doc_fetcher.fetch_and_clean('{{cookiecutter.allowed_domains.split(",")[0].strip()}}/page')

        # Assert
        assert result.url == '{{cookiecutter.allowed_domains.split(",")[0].strip()}}/page'
    ...
    def test_fetch_and_clean_plain_text_content(self, mock_get):
        ...
        # Act
        result = doc_fetcher.fetch_and_clean('{{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc.txt')

        # Assert
        assert result.url == '{{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc.txt'
    ...
    def test_fetch_and_clean_markdown_content(self, mock_get):
        ...
        # Act
        result = doc_fetcher.fetch_and_clean('{{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc.md')

        # Assert
        assert result.url == '{{cookiecutter.allowed_domains.split(",")[0].strip()}}/doc.md'
    ...
    def test_fetch_and_clean_html_without_title(self, mock_get):
        ...
        # Act
        result = doc_fetcher.fetch_and_clean('{{cookiecutter.allowed_domains.split(",")[0].strip()}}/page.html')

        # Assert
        assert result.url == '{{cookiecutter.allowed_domains.split(",")[0].strip()}}/page.html'
